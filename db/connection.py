...
def load_config():
    ...
    # Try config/ folder first, then project root, then same directory
    search_paths = [
        Path(__file__).parent.parent / "config" / "db-config.json",
        Path(__file__).parent.parent / "db-config.json",
        Path(__file__).parent / "db-config.json",
    ]
    for config_path in search_paths:
        if config_path.exists():
            with open(config_path, "r") as f:
                return json.loads(f.read())
    print("Error reading file from db-config.json - not found in any search path")
